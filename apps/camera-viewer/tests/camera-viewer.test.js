...
assert.strictEqual(appInfo.version, '0.3.21');
assert.strictEqual(appInfo.type, 'web');
assert.strictEqual(appInfo.transparent, false);
assert.strictEqual(appInfo.defaultWindowType, 'card');
...
assert.strictEqual(/id="screen-guard"/.test(html), false, 'A full app ne tartson rejtett fallback videót.');
assert.strictEqual(/\/screen-guard\.mp4/.test(script), false, 'A kamera app ne indítson második videó-dekódert screen guardként.');
...
assert.ok(/GRID_LIVE_FOCUS_DELAY_MS\s*=\s*1000/.test(script), 'A launcherhez hasonlóan csak stabil fókusz után induljon MJPEG stream.');
...
assert.ok(/function directionalTileScore\(from, to, keyCode\)/.test(script) && /data-grid-x/.test(script), 'A kiemelt 2×2 kamera navigációja előre számolt rácskoordinátákat használjon.');
assert.strictEqual(/getBoundingClientRect\(\)/.test(script), false, 'Nyílgombnál ne legyen kényszerített layout-mérés.');
...
assert.ok(/animation:\s*none\s*!important/.test(css), 'A TV-s kamera appban az animációk legyenek kikapcsolva.');
assert.ok(/transition:\s*none\s*!important/.test(css), 'A fókuszváltás ne animáljon.');
assert.ok(/defaultWindowType/.test(JSON.stringify(appInfo)) && appInfo.defaultWindowType === 'card');
assert.strictEqual(/Valódi videó/.test(html + script), false, 'A mód neve egyszerűen Videó legyen.');
