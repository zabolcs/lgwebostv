(function () {
  ...
  function updateItemLinkFields() {
    var found = findItem(el('launcher-admin-item-id').value);
    var link = !!found && found.item.type === 'link';
    var mode = el('launcher-admin-item-link-mode').value || 'website';
    var method = el('launcher-admin-item-webhook-method').value || 'GET';
    el('launcher-admin-item-link-mode-field').hidden = !link;
    el('launcher-admin-item-link-url-field').hidden = !link;
    el('launcher-admin-item-webhook-method-field').hidden = !link || mode !== 'webhook';
    el('launcher-admin-item-webhook-body-field').hidden = !link || mode !== 'webhook' || method !== 'POST';
  }
  function openItemEditor(itemId) {
    ...
    el('launcher-admin-item-link-mode').value = item.linkMode === 'webhook' ? 'webhook' : 'website'; el('launcher-admin-item-link-url').value = item.type === 'link' ? item.targetId : '';
    el('launcher-admin-item-webhook-method').value = item.webhookMethod === 'POST' ? 'POST' : 'GET'; el('launcher-admin-item-webhook-body').value = item.webhookBody || '';
    updateItemLinkFields();
    el('launcher-admin-item-overlay').hidden = false; el(item.type === 'link' ? 'launcher-admin-item-link-mode' : 'launcher-admin-item-label').focus();
  }
  ...
  function saveItemEditor(event) {
    ...
    if (item.type === 'link') {
      item.targetId = el('launcher-admin-item-link-url').value.trim();
      if (!item.targetId) { setStatus('A webcím / webhook URL kitöltése kötelező.', true); return; }
      item.linkMode = el('launcher-admin-item-link-mode').value === 'webhook' ? 'webhook' : 'website';
      item.webhookMethod = item.linkMode === 'webhook' && el('launcher-admin-item-webhook-method').value === 'POST' ? 'POST' : 'GET';
      item.webhookBody = item.linkMode === 'webhook' && item.webhookMethod === 'POST' ? el('launcher-admin-item-webhook-body').value : '';
    }
  ...
  function updateAddLinkFields() {
    var type = targetType(el('launcher-add-row').value);
    var link = type === 'link';
    var mode = el('launcher-add-link-mode').value || 'website';
    var method = el('launcher-add-webhook-method').value || 'GET';
    el('launcher-add-link-mode-field').hidden = !link;
    el('launcher-add-url-field').hidden = !link;
    el('launcher-add-webhook-method-field').hidden = !link || mode !== 'webhook';
    el('launcher-add-webhook-body-field').hidden = !link || mode !== 'webhook' || method !== 'POST';
  }
  function fillAddTargets() {
    ...
    select.parentNode.hidden = type === 'link' || type === 'allApps' || type === 'settings';
    select.textContent = '';
  ...
  function addItem() {
    ...
    if (type === 'link') {
      item.linkMode = el('launcher-add-link-mode').value === 'webhook' ? 'webhook' : 'website';
      item.webhookMethod = item.linkMode === 'webhook' && el('launcher-add-webhook-method').value === 'POST' ? 'POST' : 'GET';
      item.webhookBody = item.linkMode === 'webhook' && item.webhookMethod === 'POST' ? el('launcher-add-webhook-body').value : '';
    }
    row.items.push(item);
    el('launcher-add-label').value = ''; el('launcher-add-url').value = ''; el('launcher-add-link-mode').value = 'website'; el('launcher-add-webhook-method').value = 'GET'; el('launcher-add-webhook-body').value = ''; renderRowsEditor(); preview(); setStatus('Az elem bekerült az előnézetbe; a véglegesítéshez mentsd a beállításokat.');
  }
  ...
  function buildEditor() {
    ...
    el('launcher-admin-item-link-mode').addEventListener('change', updateItemLinkFields); el('launcher-admin-item-webhook-method').addEventListener('change', updateItemLinkFields);
    ...
    el('launcher-add-row').addEventListener('change', fillAddTargets); el('launcher-add-type').addEventListener('change', fillAddTargets); el('launcher-add-link-mode').addEventListener('change', updateAddLinkFields); el('launcher-add-webhook-method').addEventListener('change', updateAddLinkFields); el('launcher-add-button').addEventListener('click', addItem);
