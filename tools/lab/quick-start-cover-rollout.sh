...
grep -q "guard v0.4.3" tools/generated-tv-scripts/guard.sh
...
start_guard
sleep 1
"${SSH[@]}" "tail -30 /tmp/hu.szabi.launcher-wake.log 2>/dev/null | grep -q 'guard v0.4.3 started'"
...
if cover >= 2.20:
    raise SystemExit(f"Prepare Resume cover did not beat protected 2.368s baseline enough: {cover:.3f}s")
if full-cover < 1.0:
...
"${SSH[@]}" "tail -100 /tmp/hu.szabi.launcher-wake.log 2>/dev/null | grep -q 'prepare resume cover accepted'"
