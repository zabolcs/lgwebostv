...
service.register('armBootup', function (message) {
  var manager;
  try { manager = core.managerUri(message.payload); }
  catch (error) { failure(message, 22, error.message); return; }
  service.call(manager + '/create', core.bootupCreateRequest(), function (response) {
    var result = core.publicResult(response && response.payload);
    var state = readState();
    state.lastBootupArm = {epochMs: Date.now(), manager: manager, returnValue: result.returnValue,
      activityId: result.activityId || null, errorCode: result.errorCode === undefined ? null : result.errorCode};
    writeState(state);
    message.respond(result);
  });
});
