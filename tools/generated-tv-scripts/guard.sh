#!/bin/sh
# v0.4.5 guard: Screen On cover plus unchanged v0.4.0 full-launch fallback.
...
QUICK_SCREENON_ATTEMPT=/tmp/hu.szabi.launcher.quick-screenon-attempt
QUICK_SCREENON_ACCEPTED=/tmp/hu.szabi.launcher.quick-screenon-accepted
...
settle_wake() {
  ...
  rm -f "$POWER_STARTUP" "$WAKE_WAIT_UNTIL" "$WAKE_RETRY_UNTIL" "$POWER_OFF_APP" "$WAKE_VISIBLE_APP" "$WAKE_VISIBLE_SINCE" "$QUICK_WAKE_ARMED" "$QUICK_FAST_ATTEMPT" "$QUICK_SCREENON_ATTEMPT" "$QUICK_SCREENON_ACCEPTED"
}
...
screenon_cover_worker() {
  [ -f "$QUICK_WAKE_ARMED" ] || { rm -f "$QUICK_SCREENON_ATTEMPT"; return 1; }
  [ -f "$QUICK_COVER_READY" ] || { rm -f "$QUICK_SCREENON_ATTEMPT"; return 1; }
  [ ! -f "$HOME_ACTIVE" ] || { rm -f "$QUICK_SCREENON_ATTEMPT"; return 1; }
  if ! quick_fast_lane_safe; then
    rm -f "$QUICK_SCREENON_ATTEMPT"
    diagnostic 'screen on cover blocked: EIM overlay not healthy'
    return 1
  fi
  origin=$(cat "$CONTROL_ORIGIN" 2>/dev/null)
  display=$(cat "$DIR/display-preferences.json" 2>/dev/null); [ -n "$display" ] || display='{}'
  payload=$(printf '{"id":"%s","noSplash":true,"params":{"source":"quick-start-screen-on","launcherHost":"full-overlay","controlOrigin":"%s","displayPreferences":%s}}' "$OVERLAY_APP" "$origin" "$display")
  diagnostic 'screen on cover dispatch'
  ...
  if echo "$result" | grep -Eq '"returnValue"[[:space:]]*:[[:space:]]*true'; then
    touch "$QUICK_SCREENON_ACCEPTED"
    diagnostic 'screen on cover accepted'
    return 0
  fi
  rm -f "$QUICK_SCREENON_ATTEMPT"
  diagnostic 'screen on cover failed; v0.4.0 fallback remains armed'
  return 1
}
...
quick_start_fast_launch() {
  ...
  if [ -f "$QUICK_COVER_READY" ] && [ ! -f "$QUICK_SCREENON_ACCEPTED" ]; then
...
handle_power_state() {
  ...
  case "$state" in
    Active)
      ...
      ;;
    'Screen Saver')
      rm -f "$POWER_STARTUP" "$WAKE_WAIT_UNTIL" "$WAKE_RETRY_UNTIL" "$QUICK_WAKE_ARMED" "$QUICK_FAST_ATTEMPT" "$QUICK_SCREENON_ATTEMPT" "$QUICK_SCREENON_ACCEPTED"
      ;;
    'Active Standby'|Suspend|'Screen Off')
      [ "$previous" = Active ] && snapshot_power_off_app
      touch "$QUICK_WAKE_ARMED"
      rm -f "$QUICK_FAST_ATTEMPT" "$QUICK_SCREENON_ATTEMPT" "$QUICK_SCREENON_ACCEPTED" "$BOOT_READY" "$POWER_STARTUP" "$WAKE_WAIT_UNTIL" "$WAKE_RETRY_UNTIL"
      ;;
    *)
      [ "$previous" = Active ] && snapshot_power_off_app
...
power_loop() {
  while [ -f "$ENABLED" ]; do
    ...
    while IFS= read -r line; do
      ...
      if [ "$processing" = 'Screen On' ]; then
        # This TV emits Screen On about 90 ms before Active. Activate only the
        # already-prewarmed cover here; the normal v0.4.0 Active path remains
        # fully armed if SAM/compositor is not ready yet.
        if [ -f "$QUICK_WAKE_ARMED" ] && [ -f "$QUICK_COVER_READY" ] &&
           [ ! -f "$QUICK_SCREENON_ATTEMPT" ] && [ ! -f "$QUICK_SCREENON_ACCEPTED" ]; then
          touch "$QUICK_SCREENON_ATTEMPT"
          screenon_cover_worker 9>&- &
        fi
      fi
...
rm -f "$ALLOW" "$POWER_STATE" "$POWER_EVENT" "$FOREGROUND_EVENT" "$BOOT_READY" "$LAST_LAUNCH" "$WAKE_SIGNAL" "$QUICK_WAKE_ARMED" "$QUICK_FAST_ATTEMPT" "$QUICK_SCREENON_ATTEMPT" "$QUICK_SCREENON_ACCEPTED"
foreground_loop 9>&- &
power_loop 9>&- &
wake_gap_loop 9>&- &
diagnostic 'guard v0.4.5 started'
next_poll=0
while [ -f "$ENABLED" ]; do
  run_pending_tick
  /bin/usleep 250000
